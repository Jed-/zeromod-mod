    ...
    void startintermission() {
    	...
    	if(sendscores && dlcmd && dlcmd[0] && scoreboardurl && scoreboardurl[0] && !m_edit) {
    		loopv(clients) {
    			...
    			clientinfo *ci = clients[i];
    			defformatstring(cmd)("%s \"%s?a=g&n=%s&k=%d&d=%d&tots=%d&totd=%d&s=%d&t=%d&l=%d\"", dlcmd, scoreboardurl, ci->name, ci->state.frags, ci->state.deaths, ci->state.shotdamage, ci->state.damage, ci->state._suicides, ci->state.teamkills, ci->state.flags);
    			system(cmd);
    		}
    	}
    }
