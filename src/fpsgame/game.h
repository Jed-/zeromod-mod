...
static int _roundstart = 0;
static int _roundsgood = 0;
static int _roundsevil = 0;
static int _scored = 0;

static bool _match = false;
static int _matchstart = 0;
static int _matchmsg = 0;
static vector<int> _matchclients;

static bool _loaded_map = false;
static bool _force_bot = false;
...
} gamemodes[] =
{
    ...
    { "insta team", M_NOITEMS | M_INSTA | M_TEAM | M_OVERTIME, "Instagib Team: You spawn with full rifle ammo and die instantly from one shot. There are no items. Frag \fs\f3the enemy team\fr to score points for \fs\f1your team\fr." },
    ...
    { "effic team", M_NOITEMS | M_EFFICIENCY | M_TEAM | M_OVERTIME, "Efficiency Team: You spawn with all weapons and armour. There are no items. Frag \fs\f3the enemy team\fr to score points for \fs\f1your team\fr." },
    ...
    { "tac team", M_NOITEMS | M_TACTICS | M_TEAM | M_OVERTIME, "Tactics Team: You spawn with two random weapons and armour. There are no items. Frag \fs\f3the enemy team\fr to score points for \fs\f1your team\fr." },
...
struct fpsstate
{
    ...
    fpsstate() : maxhealth(100), aitype(AI_NONE), skill(0), wp1(6), wp2(0) {}
...
namespace server
{
    ...
    extern void startarena(const char *map, int mode);
    extern void checkarena();
    extern void checkstartround();
    extern void _respawn();
    extern void checkloadmap();
    extern void startround();
    extern void checkround();
    extern void startmatch(int mode, const char *map);
    ...
    extern bool protecteduserlogin(int cn, const char *authname, const char *authdesc);
    extern bool protectedclanuserlogin(int cn, const char *authname, const char *authdesc);
    extern bool isreservedname(char *name);
    extern bool isreservedclan(char *name);
    extern char *reserveddomain(char *name);
    extern void checkprotection(int cn);
    extern void addreservedname(char *nick, const char *authname);
    extern void addreservedclan(char *tag, const char *authdesc);
    extern char *reservedauthname(char *name);
    extern char *reserveddomain(char *name);
