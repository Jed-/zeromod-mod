...
namespace server
{
    ...
    namespace aiman
    {
        ...
        extern void reqdel(clientinfo *ci, bool force = false);
...
/*		loopi(argc) {
    ...
    {
        if(!name) name = ci->name;
        if(name[0] && !duplicatename(ci, name) && (ci->state.aitype == AI_NONE || ci->state.state==CS_SPECTATOR)) return name;
        static string cname[3];
        static int cidx = 0;
        cidx = (cidx+1)%3;
        formatstring(cname[cidx])(ci->state.aitype == AI_NONE || ci->state.state==CS_SPECTATOR ? "%s \fs\f1(%d)\fr" : "%s \fs\f1[%d]\fr", name, ci->clientnum);
        return cname[cidx];
    }
...
//                loopv(clients) if(ci!=clients[i] && clients[i]->privilege)
        ...
        if((ci->privilege && !ishidden && ((oldpriv && washidden) || (!oldpriv && !washidden))) || (oldpriv && !washidden))
        {
            ...
            {
                string tmp;
                formatstring(tmp)("\f0[\f7Priv\f0]\f7 %s %s %s", colorname(ci), "claimed", privname(ci->privilege));
                sendstring(tmp, p);
            }
            else if(ishidden)
            {
                defformatstring(tmp)("\f0[\f7Priv\f0]\f7 %s %s %s", colorname(ci), "relinquished", privname(oldpriv));
                sendstring(tmp, p);
            }
...
/*    	if(sendscores && dlcmd && dlcmd[0] && scoreboardurl && scoreboardurl[0] && !m_edit) {
    ...
    ICOMMAND(msg, "C", (char *s), sendf(-1, 1, "ris", N_SERVMSG, s));
    
    void _stoprampage(clientinfo *_ci, clientinfo *_a) {
    	int n = _ci->state.rampage;
    	if(n >= 15 && !m_edit) {
    		defformatstring(msg)("\f0%s\f7 has been stopped by \f0%s\f7!", colorname(_ci), _ci->clientnum==_a->clientnum ? _ci->gender ? "herself" : "himself" : colorname(_a));
    		sendf(-1, 1, "ris", N_SERVMSG, msg);
    	}
    	_ci->state.rampage = 0;
    	_ci->state.multikills = 0;
    }
...
//    		if(isreservedname(clients[i]->name) && !clients[i]->logged) checkreservedname(clients[i]->clientnum);
    ...
    {
        //bannedips.shrink(0);
		// loopvrev(bannedips) if(!bannedips[i].perm) bannedips.remove(i); // no longer clearing bans
        aiman::clearai();
        persist = persistteams;
        autosendmap = 0;
...
*/
    ...
    void receivefile(int sender, uchar *data, int len)
    {
        ...
        {
            sendf(sender, 1, "ris", N_SERVMSG, "\f3[\f7Mute\f3] \f7your sendmap was \f3muted");
            return;
        }
    ...
    bool connected(clientinfo *ci)
    {
        ...
        if(_wpmode) {
        	sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f1[\f7Wp\f1]\f7 weapon preference: use \f0#setwp\f7 when you're dead to get \f62\f7 weapons! - Codes: cs, sg, cg, rl, ri, gl, pi");
        }
        if(_arena) {
...
//      int num;
    ...
    void _setcountry(int cn, const char *country) {
    	...
    	string _country;
    	copystring(_country, country, MAXSTRLEN);
    	decodeutf8(_buf, sizeof(_buf), (uchar*)_country, MAXSTRLEN, NULL);
    ...
    void _setregion(int cn, const char *region) {
    	...
    	uchar _buf[MAXSTRLEN];
    	string _region;
    	copystring(_region, region, MAXSTRLEN);
    	decodeutf8(_buf, sizeof(_buf), (uchar*)_region, MAXSTRLEN, NULL);
    ...
    void _settown(int cn, const char *town) {
    	...
    	uchar _buf[MAXSTRLEN];
    	string _town;
    	copystring(_town, town, MAXSTRLEN);
    	decodeutf8(_buf, sizeof(_buf), (uchar*)_town, MAXSTRLEN, NULL);
    ...
    void _load(const char *cmd, const char *args, clientinfo *ci)
    {
        ...
        for(char *s = argv[0]; *s; s++)
            for(char *p = badchars; *p; p++)
                if(*s == *p)
                {
                    _notify("\f3[\f7Fail\f3] \f7invalid module name", ci);
                    if(!ci) logoutf("\f3[\f7Fail\f3] \f7invalid module name");
                    return;
                }
        ...
        if(needload)
        {
            ...
            if(!initfunc)
            {
                defformatstring(msg)("\f3[\f7Fail\f3] \f7plugin \f0%s \f7symbol \f0z_init \f7lookup failed \f3(\f7%s\f3)", argv[0], z_liberror());
                _notify(msg, ci, PRIV_ROOT);
                logoutf("%s", msg);
                Z_FREELIB(m->h);
                _modules.remove(mi);
    ...
    */
    ...
    void _pm(const char *cmd, const char *args, clientinfo *ci)
    {
        ...
        if(ci->_xi.mute)
        {
            sendf(ci->ownernum, 1, "ris", N_SERVMSG, "\f3[\f7Mute\f3] \f7you are \f3muted");
            return;
        }
    ...
    void _setpriv(const char *cmd, const char *args, clientinfo *ci)
    {
            ...
            (_getpriv(ci)>=privilege)))
        {
            ...
            if((privilege && !ishidden && ((oldpriv && washidden) || (!oldpriv && !washidden))) || (oldpriv && !washidden))
            {
                ...
                if(washidden)
                {
                    defformatstring(tmp)("\f0[\f7Priv\f0]\f7 %s %s %s", colorname(cx), "claimed", privname(privilege));
                    sendstring(tmp, p);
                }
                else if(ishidden)
                {
                    defformatstring(tmp)("\f0[\f7Priv\f0]\f7 %s %s %s", colorname(cx), "relinquished", privname(oldpriv));
                    sendstring(tmp, p);
                }
                else sendstring(msg, p);
...
//            concatstring(msg, buf);
    ...
    void _votekickfunc(const char *cmd, const char *args, clientinfo *ci)
    {
        ...
        if(!_votekick(ci, cn)) sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3[\f7Fail\f3] \f7votekick \f3failed");
    }
...
/*    	if(ci->state.state==CS_ALIVE && ci->wpchosen) {
    ...
    void _matchfunc(const char *cmd, const char *args, clientinfo *ci) {
        ...
        defformatstring(msg)("\f1[\f7Match\f1]\f7 starting match: \f0%s %s", modenum >= 0 && modenum != 1 ? _modenames[modenum] : _modenames[gamemode], map && map[0] ? map : smapname);
        sendf(-1, 1, "ris", N_SERVMSG, msg);
    ...
    void _cwfunc(const char *cmd, const char *args, clientinfo *ci) {
        ...
        defformatstring(msg)("\f1[\f7Match\f1]\f7 starting ClanWar: \f1%s\f7 against \f3%s\f7 on \f0%s %s", c1, c2, modenum >= 0 && modenum != 1 ? _modenames[modenum] : _modenames[gamemode], map && map[0] ? map : smapname);
        sendf(-1, 1, "ris", N_SERVMSG, msg);
        startmatch(modenum >= 0 && modenum != 1 ? modenum : gamemode, map && map[0] ? map : smapname);
        loopv(clients) {
        	if(strstr(clients[i]->name, c1)) {
        		copystring(clients[i]->team, "good", MAXTEAMLEN+1);
            	sendf(!clients[i]->_xi.spy ? -1 : clients[i]->ownernum, 1, "riisi", N_SETTEAM, clients[i]->clientnum, "good", -1);
            } else if(strstr(clients[i]->name, c2)) {
            	copystring(clients[i]->team, "evil", MAXTEAMLEN+1);
            	sendf(!clients[i]->_xi.spy ? -1 : clients[i]->ownernum, 1, "riisi", N_SETTEAM, clients[i]->clientnum, "evil", -1);
            } else {
            	clients[i]->state.state = CS_SPECTATOR;
            	sendf(-1, 1, "ri3", N_SPECTATOR, clients[i]->clientnum, 1);
            }
        }
    }
    void _sharefunc(const char *cmd, const char *args, clientinfo *ci) {
    	if(!ci) return;
    	loopv(clients) {
    		if(clients[i]!=ci && clients[i]->privilege<PRIV_MASTER && clients[i]->state.state!=CS_SPECTATOR && clients[i]->state.aitype==AI_NONE) {
//    			setmaster(clients[i], true);
...
//    	aiman::reqdel(ci, true);
    ...
    void _beerfunc(const char *cmd, const char *args, clientinfo *ci) {
		...
		int numbeers = -1;
		if(!argv[1] || !argv[1][0]) numbeers = 1; else {
			int _n = atoi(argv[1]);
			if(_n <= 0) {
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3[\f7Error\f3] \f7The minimum number of beers is 1!");
				return;
			}
			numbeers = _n;
		}
...
// ****************************************************************************************
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_TRYSPAWN:
            	if(_arena) {
            		if(ci->state.aitype==AI_NONE) sendf(sender, 1, "ris", N_SERVMSG, "\f1[\f7Arena\f1]\f7 you can't respawn until the end of this round!");
            		break;
            	}
            ...
            case N_TEXT:
            {
                ...
                /* if(ci->_xi.msgnum >= 80) // disabling the antiflood to let everybody speak
                {
                ...
                else
                {
                    ...
                    {
                        //cq->messages.drop();
                        sendf(sender, 1, "ris", N_SERVMSG, "\f3[\f7Mute\f3] \f7you are \f3muted");
                    }
                    else
                    {
                        logoutf("%s: %s", colorname(cq), ftext);
            ...
            case N_SAYTEAM:
            {
                ...
                if(ci->_xi.mute || cq->_xi.mute)
                {
                    sendf(sender, 1, "ris", N_SERVMSG,
                          "\f3[\f7Mute\f3] \f7you are \f3muted");
                    break;
                }
...
//                ci->playermodel = getint(p);
            ...
            case N_DELCUBE:
            {
                ...
                if(cq && (ci->_xi.editmute || cq->_xi.editmute))
                {
                    ...
                    {
                        sendf(sender, 1, "ris", N_SERVMSG,
                              "\f3[\f7Mute\f3] \f7your editing is \f3muted");
            ...
            case N_NEWMAP:
            {
                ...
                if(ci->_xi.editmute)
                {
                    ...
                    {
                        sendf(sender, 1, "ris", N_SERVMSG,
                              "\f3[\f7Mute\f3] \f7your \f0newmap\f7 was \f3muted");
                        ci->_xi.editmutewarn = totalmillis ? totalmillis : 1;
                    }
                    break;
                }
